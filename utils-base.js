...
(function(utils, log) {
    ...
    if ((typeof(utils) != "object") && (typeof(utils) != "function")) {
        log.error("Utils", "no valid utils object given, unable to extend");
        return;
    }
    ...
    var _mustNOTexist = function(name) {
        if ((utils[name] !== null) && (utils[name] !== undefined)) {
            log.error("Utils : property or method {0} already exists, not overwriting".fmt(name));
        }
    };
    var _mustExist = function(name) {
        if ((utils[name] === null) || (utils[name] === undefined)) {
            log.error("Utils : property or method {0} does not exists, will not be available".fmt(name));
        }
    };
    ...
    utils.get = utils.get || function(obj, path, objName) {
        ...
        if (!utils.obj(obj)) {
            if (describe) { log.error(me, "object {0} is invalid, can't get property {1}".fmt(objName, path)); }
            return value;
        }
        ...
        for (var idx = 0; idx < numProps; idx++) {
            ...
            if (describe) { pathTravelled += ((idx > 0) ? '.' : '') + prop; }
        }
    ...
    utils.set = utils.set || function(obj, path, value, objName) {
        ...
        if (!utils.obj(obj)) {
            if (describe) { log.error(me, "object {0} is invalid, can't set property {1}".fmt(objName, path)); }
            return success;
        }
